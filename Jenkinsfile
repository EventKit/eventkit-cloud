...
"""
    ...
    stage("Run linter"){
        try{
            ...
            sh "docker-compose run --rm -T  webpack npm run eslint --config /var/lib/eventkit/config/ui/eslintrc.json"
            sh "docker-compose run --rm webpack tslint tslint --config /var/lib/eventkit/config/ui/tslint.json --check /var/lib/eventkit/tslint.json"
