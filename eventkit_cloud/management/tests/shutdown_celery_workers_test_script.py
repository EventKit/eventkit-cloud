...
def test_shutdown_celery_workers_mgmt_cmd():
    ...
    worker_nodename = [
        n for n in list(app.control.inspect().ping().keys()) if "worker" in n
    ][0]
    worker_hostname = worker_nodename.split("@")[1]

    # Fire off a number of tasks & wait for them to be picked up
    tc1 = test_chain.apply_async(
        kwargs={"subtask_queuename": worker_hostname}, expires=6
    )
    tc1.get()
    ...
    if tc1.status != "SUCCESS":
        return (
            100,
            'Tasks started before "finish_worker_tasks", status "{}" != "SUCCESS"'.format(
                tc1.status
            ),
        )
    if not chain_success(tc1):
        return (
            100,
            'Tasks started before "finish_worker_tasks", did not all complete successfully',
        )
    if tc2.status != "PENDING":
        return (
            100,
            'Tasks started after "finish_worker_tasks", status {} != "PENDING"'.format(
                tc2.status
            ),
        )
...
if __name__ == "__main__":
    ...
    if n_preexisting_tasks != 0:
        return_code, msg = (
            100,
            "Found {} preexisting tasks, please clean up queue before executing this test",
        )
    else:
