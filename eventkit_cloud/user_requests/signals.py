...
@receiver(post_save, sender=AoiIncreaseRequest)
def aoi_request_post_save(sender, instance, created, **kwargs):
    ...
    if rocketchat_notifications:
        ...
        if created:
            message = f"@here: A new AOI request, {instance.uid} has been submitted by {instance.user}."
        else:
            message = (
                f"@here: An AOI request, {instance.uid} has been updated and is now {instance.get_status_display()}."
            )
