...
class TestUIViews(TestCase):
    def setUp(self):
        ...
        with patch("eventkit_cloud.jobs.signals.Group") as mock_group:
            ...
            self.user = User.objects.create_user(username="user", email="user@email.com", password="pass")
        self.client = Client()
    ...
    def test_covert_to_geojson(self, file_to_geojson):
            ...
            "features": [
                {
                    ...
                    "geometry": {
                        "type": "Polygon",
                        "coordinates": [[[7.2, 46.2], [7.6, 46.2], [7.6, 46.6], [7.2, 46.6], [7.2, 46.2]]],
                    },
                }
            ],
        }
        ...
        response = self.client.post("/file_upload", {"file": file_upload})
        self.assertEqual(response.status_code, 200)
    ...
    def test_search(self, mock_geocode, mock_reverse, mock_convert, mock_is_mgrs, mock_is_lat_lon):
        ...
        with self.settings(CONVERT_API_URL="url", REVERSE_GEOCODING_API_URL="url"):
            ...
            convert.get.return_value = {"geometry": {"coordinates": [1, 1]}}
            feature = {
                "geometry": {"coordinates": [1, 1]},
                "properties": {"bbox": [1 - 0.05, 1 - 0.05, 1 + 0.05, 1 + 0.05]},
                "source": "MGRS",
            }
        ...
        with self.settings(REVERSE_GEOCODING_API_URL="url"):
            ...
            point_feature = {
                ...
                "source": "Coordinate",
                "type": "Feature",
                "properties": {"name": "1 N, 1 E", "bbox": [1 - 0.05, 1 - 0.05, 1 + 0.05, 1 + 0.05]},
            }
