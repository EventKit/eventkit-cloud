...
class TestFeatureSelection(unittest.TestCase):
    ...
    def test_key_union_and_filters(self):
        ...
        self.assertEqual(
            f.filter_clause("waterways"), '"name" IS NOT NULL OR "waterway" IS NOT NULL'
        )
    ...
    def test_sql_list(self):
        ...
        f = FeatureSelection(y)
        self.assertEqual(
            f.filter_clause("waterways"), "name IS NOT NULL OR name = 'some building'"
        )
    ...
    def test_sqls(self):
        ...
        self.assertEqual(
            ...
            '"name" TEXT,"addr:housenumber" TEXT\n);\nINSERT INTO buildings_points(geom, osm_id,'
            '"name","addr:housenumber") select geom, osm_id,"name","addr:housenumber" from points '
            'WHERE ("name" IS NOT NULL OR "addr:housenumber" IS NOT NULL);\n',
        )
        self.assertEqual(
            create_sqls[1],
            'CREATE TABLE buildings_polygons(\nfid INTEGER PRIMARY KEY AUTOINCREMENT,\ngeom MULTIPOLYGON,\nosm_id '
            'TEXT,osm_way_id TEXT,"name" TEXT,"addr:housenumber" TEXT\n);\n'
            'INSERT INTO buildings_polygons(geom, osm_id,osm_way_id,"name","addr:housenumber") '
            'select geom, osm_id,osm_way_id,"name","addr:housenumber" from multipolygons '
            'WHERE ("name" IS NOT NULL OR "addr:housenumber" IS NOT NULL);\n',
        )
    ...
    def test_zindex(self):
        ...
        self.assertEqual(
            ...
            'TEXT,"highway" TEXT,"z_index" TEXT\n);\nINSERT INTO roads_lines(geom, osm_id,"highway","z_index") '
            'select geom, osm_id,"highway","z_index" from lines WHERE ("highway" IS NOT NULL);\n',
        )
