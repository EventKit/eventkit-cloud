...
def get_session(request, provider):
    ...
    source_config = config.get("source_config", dict(user_var=None, pass_var=None, cert_info=None))
...
def get_process(provider, session):
    ...
    service_url += "/"
    cache_key = f"{config.get('process')}-cache"
    process_json = cache.get(cache_key, None)
    if process_json is None:
        try:
            response = session.get(
                f"{service_url}processes/{config.get('process')}",
                stream=True,
            )
