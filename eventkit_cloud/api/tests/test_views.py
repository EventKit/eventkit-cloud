...
class TestJobViewSet(APITestCase):
    ...
    def test_make_job_with_export_providers(self,):
        ...
        request_data = {
            ...
            "provider_tasks": [
                {"provider": "test", "formats": formats, "min_zoom": 3, "max_zoom": 6}
            ],
            "export_providers": [
                {
                    "name": "test",
        ...
        self.assertEqual(
            provider_task.min_zoom, request_data["provider_tasks"][0]["min_zoom"]
        )
        self.assertEqual(
    ...
    def test_get_job_detail(self,):
        ...
        self.assertEqual(
            response.data["exports"][0]["formats"][0]["url"],
            data["exports"][0]["formats"][0]["url"],
        )

    def test_get_job_detail_no_permissions(self,):
        user = User.objects.create_user(
            username="demo2", email="demo2@demo.com", password="demo"
        )
    ...
    def test_delete_job_no_permission(self,):
        ...
        self.assertEqual(
            ...
            "ADMIN permission is required to delete this job.",
        )
    ...
    def test_create_zipfile(self, create_run_mock, pickup_mock):
        ...
        request_data = {
            ...
            "selection": bbox_to_geojson([5, 16, 5.1, 16.1]),
            "provider_tasks": [
                {"provider": "OpenStreetMap Data (Generic)", "formats": formats}
            ],
            "preset": self.job.preset.id,
            "published": True,
            "tags": self.tags,
    ...
    def test_create_job_success(self, create_run_mock, pickup_mock):
        ...
        self.assertEqual(
            response.data["exports"][0]["formats"][0]["slug"],
            request_data["provider_tasks"][0]["formats"][0],
        )
        self.assertEqual(
            response.data["exports"][0]["formats"][1]["slug"],
        ...
        self.assertEqual(
            response.data["original_selection"], request_data["original_selection"]
        )
    ...
    def test_invalid_format_param(self,):
        ...
        request_data = {
            ...
            "selection": bbox_to_geojson([5, 16, 5.1, 16.1]),
            "provider_tasks": [
                {"provider": "OpenStreetMap Data (Generic)", "formats": ""}
            ],  # invalid
        }
        response = self.client.post(url, request_data, format="json")
        self.assertEqual(status.HTTP_400_BAD_REQUEST, response.status_code)
    ...
    def test_patch(self):
        ...
        request_data = {
            "featured": True,
            "published": False,
            "visibility": VisibilityState.SHARED.value,
        }
        response = self.client.patch(
            url,
            data=json.dumps(request_data),
...
class TestBBoxSearch(APITestCase):
    ...
    def test_bbox_search_success(self,):
        ...
        self.assertEqual(
            2, len(response.data)
        )  # 8 jobs in total but response is paginated
    ...
    def test_list_jobs_no_bbox(self,):
        ...
        self.assertEqual(response["Content-Language"], "en")
        self.assertEqual(
            response["Link"], '<http://testserver/api/jobs?page=2>; rel="next"'
        )
        self.assertEqual(
            2, len(response.data)
        )  # 8 jobs in total but response is paginated
...
class TestExportRunViewSet(APITestCase):
    ...
    def test_zipfile_url_s3(self):
        ...
        data_provider_task_record = DataProviderTaskRecord.objects.create(
            run=self.export_run, name="run", slug="run"
        )
        file_result = FileProducingTaskResult.objects.create(
            download_url=example_url, size=10
        )
        ...
        with self.settings(USE_S3=False):
            ...
            self.assertEqual(download_url, result[0]["zipfile_url"])
...
class TestExportTaskViewSet(APITestCase):
    ...
    def test_patch_cancel_task(self,):
        ...
        url = reverse("api:provider_tasks-list") + "/%s" % (
            self.export_provider_task.uid,
        )
        self.assertEqual(expected, url)
...
class TestStaticFunctions(APITestCase):
    ...
    def test_get_provider_tasks(self):
        ...
        export_provider = DataProvider.objects.create(
            name="provider1", export_provider_type=provider_type
        )
...
class TestUserDataViewSet(APITestCase):
    def setUp(self,):
        ...
        self.licenses = [
            License.objects.create(slug="test1", name="Test1", text="text")
        ]
        self.licenses += [
            License.objects.create(slug="test2", name="Test2", text="text")
        ]
        token = Token.objects.create(user=self.user)
        self.client.credentials(
            HTTP_AUTHORIZATION="Token " + token.key,
    ...
    def test_get_userdata_list(self):
        ...
        self.assertIsNotNone(
            data[0].get("accepted_licenses").get(self.licenses[0].slug)
        )
    ...
    def test_set_licenses(self):
        ...
        self.assertEqual(
            data.get("accepted_licenses").get(self.licenses[0].slug), False
        )
        self.assertEqual(
            data.get("accepted_licenses").get(self.licenses[1].slug), False
        )
...
class TestGroupDataViewSet(APITestCase):
    def setUp(self,):
        ...
        gp = GroupPermission.objects.create(
            group=group, user=self.user1, permission=GroupPermissionLevel.ADMIN.value
        )
    ...
    def test_insert_group(self):
        ...
        response = self.client.post(
            url, data=json.dumps(payload), content_type="application/json; version=1.0"
        )
        self.assertEqual(status.HTTP_200_OK, response.status_code)
...
class TestUserJobActivityViewSet(APITestCase):
    ...
    def setUp(self):
        ...
        for i in range(3):
            ...
            UserJobActivity.objects.create(
                user=self.user, job=job, type=UserJobActivity.VIEWED
            )
    ...
    def test_create_viewed_existing(self):
        ...
        self.assertEqual(
            viewed_jobs[-1]["last_export_run"]["job"]["uid"], str(job_a.uid)
        )
    ...
    def create_job(self, name):
        ...
        job = Job.objects.create(
            name=name, description="Test description", user=self.user, the_geom=the_geom
        )
        formats = ExportFormat.objects.all()
        provider = DataProvider.objects.first()
        provider_task = DataProviderTask.objects.create(provider=provider)
