...
class TestJobPermissions(APITestCase):
    ...
    def setUp(self,):
        ...
        self.user1 = User.objects.create_user(
            username="user_1", email="demo@demo.com", password="demo"
        )
        self.user2 = User.objects.create_user(
            username="user_2", email="demo@demo.com", password="demo"
        )
        ...
        gp = GroupPermission.objects.create(
            group=group1, user=self.user1, permission=GroupPermissionLevel.ADMIN.value
        )
        gp = GroupPermission.objects.create(
            group=group1, user=self.user2, permission=GroupPermissionLevel.MEMBER.value
        )
        group2, created = Group.objects.get_or_create(name="group_two")
        self.group2id = group2.id
        gp = GroupPermission.objects.create(
            group=group2, user=self.user1, permission=GroupPermissionLevel.ADMIN.value
        )
        gp = GroupPermission.objects.create(
            group=group2, user=self.user2, permission=GroupPermissionLevel.MEMBER.value
        )
    ...
    def test_get_job_defaults(self,):
        ...
        self.assertEqual(
            self.user1.username in response.data["permissions"]["members"], True
        )
        self.assertEqual(
            response.data["permissions"]["members"][self.user1.username], "ADMIN"
        )
        self.assertEqual(response.data["visibility"], "PRIVATE")
    ...
    def test_permissions_syntax(self,):
        ...
        request_data = {
            "permissions": {
                "members": {"user_1": "ADMIN", "user_2": "ADMIN"},
        ...
        request_data = {
            "permissions": {
                "members": {"user_1": "READ", "user_2": "READ"},
                "groups": {},
            }
        }
        ...
        request_data = {
            "permissions": {
                "members": {"user_1": "ADMIN", "user_2": "ADMEN"},
                "groups": {},
            }
        }
        ...
        request_data = {
            "permissions": {
                "members": {"user_1": "ADMIN", "user_3": "ADMIN"},
                "groups": {},
            }
        }
        ...
        self.assertEqual(status.HTTP_400_BAD_REQUEST, response.status_code)
        self.assertEqual(
            response.data[0]["detail"], "unidentified user or group : user_3"
        )

        request_data = {
            "permissions": {
                "members": {"user_1": "ADMIN"},
                "groups": {"group_three": "ADMIN"},
            }
        }
        ...
        self.assertEqual(status.HTTP_400_BAD_REQUEST, response.status_code)
        self.assertEqual(
            response.data[0]["detail"], "unidentified user or group : group_three"
        )

        request_data = {
            "permissions": {
                "members": {"user_1": "ADMIN"},
                "groups": {"group_two": "ADMEN"},
            }
        }
    ...
    def test_shared_user_permissions(self,):
        ...
        request_data = {
            "visibility": "SHARED",
            "permissions": {
                "members": {"user_1": "READ", "user_2": "ADMIN"},
                "groups": {},
            },
        }
        ...
        self.assertEqual(
            ...
            "ADMIN permission is required to update this job.",
        )
        ...
        self.assertEqual(
            response.data["permissions"]["members"][self.user1.username], "READ"
        )
    ...
    def test_private_user_permissions(self,):
        ...
        request_data = {
            "visibility": "PRIVATE",
            "permissions": {"members": {"user_2": "ADMIN"}, "groups": {}},
        }
...
class TestExportRunViewSet(APITestCase):
    ...
    def test_user_sharing(self,):
        ...
        request_data = {
            "permissions": {"members": {"user_1": "ADMIN", "user_2": "ADMIN"},}
        }
