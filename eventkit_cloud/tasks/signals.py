...
def exportrun_delete_exports(sender, instance, *args, **kwargs):
    ...
    run_dir = "{0}/{1}".format(
        settings.EXPORT_DOWNLOAD_ROOT.rstrip("/"), str(instance.uid)
    )
    try:
        ...
        logger.warn(
            "The directory {0} was already moved or doesn't exist.".format(run_dir)
        )
...
def exporttaskresult_delete_exports(sender, instance, *args, **kwargs):
    ...
    full_file_download_path = "/".join(
        [settings.EXPORT_DOWNLOAD_ROOT.rstrip("/"), url_parts[-2], url_parts[-1]]
    )
    try:
