...
def pcf_scale_celery(max_instances):
    ...
    ping_command = " && ".join(
        [f"celery inspect -A eventkit_cloud --timeout=60 --destination={hostname} ping" for hostname in hostnames])
    health_check_command = f"sleep 60; while {ping_command} >/dev/null 2>&1; do sleep 600; done; echo At least one $HOSTNAME worker is dead! Killing Task...; pkill celery"
