...
class BenchmarkEventkit(object):
    ...
    def run_tests(self, batch_sizes):
        ...
        for batch_size in batch_sizes.split(","):
            ...
            print(
                "Test for batch size: {0} finished in {1} seconds".format(
    ...
    def run_iteration(self, batch_size):
        ...
        if self.sources:
            ...
            provider_tasks = [
                {"provider": provider.get("name"), "formats": ["gpkg"]} for provider in self.client.get_providers()
            ]
        ...
        while response:
            for run in response:
                if run.get("user") == self.username:
                    job_url = run["job"]["url"]
                    if job_url:
                        self.client.client.delete(run["job"]["url"], headers={"X-CSRFToken": self.client.csrftoken})
            response = self.client.search_runs()
    ...
    def wait_for_run(self, job_uid, run_timeout=0):
        ...
        while not finished:
            ...
            response = self.client.client.get(
                self.client.runs_url, params={"job_uid": job_uid}, headers={"X-CSRFToken": self.client.csrftoken}
            ).json()
